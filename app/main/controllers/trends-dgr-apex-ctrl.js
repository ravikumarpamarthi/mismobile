'use strict';
angular.module('main')
    .controller('TrendsDgrApexCtrl', function($scope, $state, $ionicLoading, $moment, ionicDatePicker, HydroService, dashboardService, $global) {
       ...
       dashboardService.getAllPlants().then(function(res) {
        ...
        $scope.getChartsByDate = function(selectedItem, month) {
            if ($global.userRole.role == "APEX") {
                var data = {
                    plantId: selectedItem.plantDocId,
                    month: month.value,
                    year: $scope.date.getFullYear(),
                    type:"HYDRO"

                }
                HydroService.dgrTrendsApexLevel(data).then(function(res) {
                    if (res.status == "SUCCESS") {
